...
### 17. Graysky领袖特质缺少本地化键 + 键名拼写错误
**文件**:
- `common/traits/00_new_leaders_traits.txt`
- `localisation/simp_chinese/rebuild_computer_l_simp_chinese.yml`
- `localisation/english/balance_for_fallen_empire_l_english.yml`

**问题**: 3个灰风(Graysky)领袖特质定义了完整的 trait 块，但缺少对应的本地化键。同时特质键名有拼写错误：`scitist` → `scientist`，`gestle` → `gestalt`，`.tooltip` → `_tooltip`。

**修复**: 修正3个特质键名的拼写错误，为中英文各补充7个本地化键（名称、描述、custom_tooltip各3个 + 额外tooltip 1个）。

---

### 18. 废弃的区划名称和恒星基地建筑名称（4.x 重命名）
**文件**:
- `events/HM_fallen_empires.txt`
- `common/solar_system_initializers/HM_fallen_initializers.txt`

**问题**: Stellaris 4.x 中多个名称被重命名/合并：
- `district_mining_uncapped` / `district_generator_uncapped` 移除（4.x 默认不设上限）
- `district_industrial` 合并入 `district_city`
- `colossus_yards` + `titan_yards` 合并为 `capital_yards`

**修复**: 替换为4.x对应名称，合并巨像/泰坦船坞为 `capital_yards`。

---

### 19. 作用域修复: capital_scope 中 set_planet_flag 需要 Planet 作用域
**文件**: `events/00_new_events_for_balanced_fallen_empire.txt`

**问题**: Stellaris 4.x 中 `capital_scope` 返回 Colony 作用域而非 Planet。5处 `set_planet_flag` 直接在 Colony 作用域上调用，需要 Planet。

**修复**: 每处 `set_planet_flag` 包裹在 `planet = { }` 内。

---

### 20. 作用域修复: every_owned_planet 中多个调用需要 Planet 作用域
**文件**: `events/00_new_events_for_tech_fallen_empire.txt`

**问题**: 同 #19，`every_owned_planet` 迭代 Colony 作用域。`set_planet_flag`、`has_planet_flag`、`planet_event`、`remove_planet_flag` 均需 Planet 作用域。

**修复**: 5处调用包裹在 `planet = { }` 内。

---

### 21. pop_job weight modifier 中无效的 has_job 触发器
**文件**: `common/pop_jobs/00_ap_nano_jobs.txt`

**问题**: `nano_maker` 和 `gestal_nano_maker` 的 weight modifier 块中使用了 `has_job = nano_maker`。`has_job` 在原版 Stellaris 中完全不存在——原版仅使用 `has_job_type`，且仅在事件 trigger 中，从不用于 job weight modifier。

**修复**: 删除两个无效的 modifier 块。

---

### 22. mod_graygoo_kill_gater 缺少本地化键
**文件**:
- `localisation/english/balance_for_fallen_empire_l_english.yml`
- `localisation/simp_chinese/balance_for_fallen_empire_l_simp_chinese.yml`

**问题**: 灰蛊风暴对抗觉醒L星门帝国的伤害加成修正 `mod_graygoo_kill_gater` 在英文和中文中均缺少本地化。

**修复**: 添加中英文 `mod_graygoo_kill_gater` 和 `mod_graygoo_kill_gater_desc` 键。

---

### 23. triggered_pop_modifier 拼写错误
**文件**: `common/traits/00_new_species_traits_by_using_virus.txt`

**问题**: `triggered_pop_modifier` 在原版中不存在。正确键名为 `triggered_pop_group_modifier`（pop group 作用域，不在 tooltip 中自动显示）。

**修复**: `triggered_pop_modifier` → `triggered_pop_group_modifier`。

---

### 24. triggered_pop_group_modifier 中修饰符键无效 + has_pop_flag 作用域错误
**文件**: `common/traits/00_new_species_traits_by_using_virus.txt`

**问题**: 
- `pop_growth_speed` 在原版 Stellaris 中完全不存在（4.x 中为 `logistic_growth_mult`，仅用于主 modifier 块）
- `planet_jobs_produces_mult` 仅用于领袖特质，物种特质中应使用 `pop_bonus_workforce_mult`
- `army_damage_mult` 和 `habitability_floor_add` 是物种级修饰符，仅用于主 modifier 块
- `has_pop_flag` 在原版任何 trait 中均未使用，需通过 `planet` 作用域间接访问 pop

**修复**: 
- 物种级修饰符（`army_damage_mult`、`habitability_floor_add`、`logistic_growth_mult`）移至主 `modifier` 块
- `planet_jobs_produces_mult` → `pop_bonus_workforce_mult`
- `has_pop_flag` 条件通过 `planet = { any_pop = { ... } }` 标准 vanilla 模式实现

---

### 25. setup_scenarios 缺少必填字段 nomad_empire_default/nomad_empire_max
**文件**: `map/setup_scenarios/{tiny,small,medium,large,huge}.txt`

**问题**: Stellaris 4.x 新增游牧帝国(Nomad Empire)机制，所有 setup_scenario 文件需包含 `nomad_empire_default` 和 `nomad_empire_max` 字段。mod 的 5 个文件均缺少这两项。

**修复**: 从原版 4.4.x 对应文件补充缺失字段，保留 mod 特有的 `fallen_empire_max = 5` 改动。

---

### 26. GFX_graysky_trait sprite 未定义导致 inline_script 展开错误
**文件**: `interface/00_new_relics_view_for_balanced_fallen_empire.gfx`

**问题**: `leader_trait_graysky_scientist` 和 `leader_trait_graysky_commander` 的 `inline_script` 引用了 `ICON = GFX_graysky_trait`，纹理文件 `graysky_trait.dds` 虽存在，但 interface `.gfx` 文件中从未定义对应的 sprite。inline_script 展开后引用不存在的 GFX 资源导致 CWTools 报错。

**修复**: 添加 `GFX_graysky_trait` sprite 定义，指向已存在的 `gfx/interface/icons/traits/leader_trait_icons/graysky_trait.dds`。

---

### 27. projectile_gfx 错误引用 GFX sprite 而非 projectile 实体
**文件**: `common/component_templates/00_new_weapons_xl_for_balanced_section_templates.txt`

**问题**: `fallen_lost_nano_cloud_h` 打击机舱的 `projectile_gfx` 字段引用了 `"GFX_fallen_lost_nano_cloud_set"`。`projectile_gfx` 需要引用 projectile 实体（在 `gfx/projectiles/` 中定义），而非 `GFX_*` 接口 sprite。`GFX_fallen_lost_nano_cloud_set` 是 UI 图标 sprite，不可作为投射物。

**修复**: `projectile_gfx` 改为 `"strike_craft_laser_1"`（原版打击机舱 point defense 武器使用的投射物）。`icon` 字段中 `GFX_fallen_lost_nano_cloud_set` 的引用保留不变——icon 正确需要 sprite 名称。

---

### 28. ae_shield_mesh 引用未定义的 pdxmesh
**文件**: `gfx/models/ships/megastructures/CVB_ZXFJ/ZXFJ.gfx`

**问题**: `CVB_mega_shield_entity` 实体的 `pdxmesh = "ae_shield_mesh"` 引用了从未在任何 `.gfx` 文件中定义的 mesh。纹理文件和 `.asset` 实体定义均存在，但缺少对应的 `pdxmesh = { }` 声明。

**修复**: 在 `ZXFJ.gfx` 中添加 `ae_shield_mesh` 的 pdxmesh 定义，指向 mod 已有的 `ZXFJ.mesh` 文件。
...
| `events/HM_fallen_empires.txt` | 2 |
| `events/00_new_events_for_tech_fallen_empire.txt` | 2 |
...
| `common/pop_jobs/00_ap_nano_jobs.txt` | 1 |
| `common/static_modifiers/00_new_static_modifiers_for_balanced_fallen_empire.txt` | 1 |
| `common/component_templates/00_new_weapons_xl_for_balanced_section_templates.txt` | 2 |
| `common/fallen_empires/HM_fallen_empire.txt` | 1 |
| `common/traits/00_new_leaders_traits.txt` | 1 |
| `common/traits/00_new_species_traits_by_using_virus.txt` | 1 |
| `common/solar_system_initializers/HM_fallen_initializers.txt` | 1 |
...
| `localisation/simp_chinese/rebuild_computer_l_simp_chinese.yml` | 1 |
| `localisation/simp_chinese/balance_for_fallen_empire_l_simp_chinese.yml` | 1 |
| `localisation/english/balance_for_fallen_empire_l_english.yml` | 2 |
| `interface/00_new_relics_view_for_balanced_fallen_empire.gfx` | 1 |
| `gfx/models/ships/megastructures/CVB_ZXFJ/ZXFJ.gfx` | 1 |
| `map/setup_scenarios/{tiny,small,medium,large,huge}.txt` | 1 |
...
- CWTools 报告的 planet_flag / pop_flag "未定义"错误是误报——planet flag 和 pop flag 为自由格式字符串，无需预先声明
